...
pub impl Op {
fn new(data : op) -> Op { let val = Op; val }
}
...
pub impl NotifyEvent {
    fn new(data : base::Event<notify_event>) -> NotifyEvent {
        NotifyEvent { base : Event { event : data } }
}
}
...
pub impl<'s> QueryExtentsCookie<'s> {
    fn new(data : base::Cookie<'s, query_extents_cookie>) -> QueryExtentsCookie {
        QueryExtentsCookie { base : data }
}
}
...
pub impl QueryExtentsCookie {
    fn new(data : base::Reply<query_extents_request>) -> QueryExtentsRequest {
        QueryExtentsRequest { base : data }
}
}
...
pub impl<'s> InputSelectedCookie<'s> {
    fn new(data : base::Cookie<'s, input_selected_cookie>) -> InputSelectedCookie {
        InputSelectedCookie { base : data }
}
}
...
pub impl InputSelectedCookie {
    fn new(data : base::Reply<input_selected_request>) -> InputSelectedRequest {
        InputSelectedRequest { base : data }
}
}
pub struct  GetRectanglesCookie<'s> { pub base : base::Cookie<'s, get_rectangles_cookie> }

pub impl<'s> GetRectanglesCookie<'s> {
    fn new(data : base::Cookie<'s, get_rectangles_cookie>) -> GetRectanglesCookie {
        GetRectanglesCookie { base : data }
}
}
/** Opcode for xcb_shape_get_rectangles. */
pub static XCB_SHAPE_GET_RECTANGLES : u8 = 8;

impl Iterator<Op> for OpIterator {
    fn next(&mut self) -> Option<Op> {
        if self.rem == 0 { return None; }
        unsafe {
            let iter : *mut op_iterator = self;
            let data = (*iter).data;
...
pub impl Kind {
fn new(data : kind) -> Kind { let val = Kind; val }
}

impl Iterator<Kind> for KindIterator {
    fn next(&mut self) -> Option<Kind> {
        if self.rem == 0 { return None; }
        unsafe {
            let iter : *mut kind_iterator = self;
            let data = (*iter).data;
...
impl NotifyEvent {
  ...
  pub fn new(shape_kind : Kind,
    ...
    unsafe {
      ...
      (*raw).shaped = shaped;
      NotifyEvent::new(raw as *mut notify_event)
    }
  }
}
...
pub fn QueryExtents<'r> (c : &'r Connection,
  ...
  unsafe {
    ...
    QueryExtentsCookie::new(Cookie {cookie:cookie,conn:c,checked:false})
  }
}
...
pub fn InputSelected<'r> (c : &'r Connection,
  ...
  unsafe {
    let cookie = xcb_shape_input_selected(c.get_raw_conn(),
        destination_window as ffi::xproto::window); //1
    InputSelectedCookie::new(Cookie {cookie:cookie,conn:c,checked:false})
  }
}
...
pub impl GetRectanglesCookie {
    fn new(data : base::Reply<get_rectangles_request>) -> GetRectanglesRequest {
        GetRectanglesRequest { base : data }
}
}
pub fn GetRectangles<'r> (c : &'r Connection,
  ...
  unsafe {
    ...
    GetRectanglesCookie::new(Cookie {cookie:cookie,conn:c,checked:false})
  }
}
